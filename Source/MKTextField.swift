...
open class MKTextField : UITextField {
    ...
    @IBInspectable open var padding: CGSize = CGSize(width: 5, height: 3)
    ...
    @IBInspectable open var layerBorderColor: Bool = false {
        didSet {
            disableBorderColor()
        }
    }
    @IBInspectable open var disableTextField: Bool = false {
        didSet {
    ...
    // floating label
    @IBInspectable open var floatingLabelFont: UIFont =  UIFont.systemFont(ofSize: 10.0) {
        didSet {
